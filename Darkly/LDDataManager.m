...
- (void)compareConfigForUser:(LDUserModel *)user withNewUser:(LDUserModel *)newUser {
    for (NSString *key in [newUser.flagConfig dictionaryValueIncludeNulls:NO]) {
        NSObject *userVal = [user.flagConfig flagValueForFlagKey:key];
        if(user == nil || (userVal != nil && ![[newUser.flagConfig flagValueForFlagKey:key] isEqual:[user.flagConfig flagValueForFlagKey:key]])) {
